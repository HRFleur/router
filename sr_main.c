...
int main(int argc, char **argv) {
	...
	while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:I:E:R:ni:")) != EOF) {
		switch (c) {
		case 'h':
			usage(argv[0]);
			exit(0);
		...
		case 'I':
			...
			break;
		case 'E':
			timeout_tcpEstablished = atoi((char *) optarg);
			break;
		case 'R':
			timeout_tcpTrans = atoi((char *) optarg);
			break;
		case 'n':
			nat_enable = 1;
			break;
		case 'i': // the ip address connect to internal network, which should be hidden.
			ip_nat = optarg;
			break;
//		case 'e': // the interface connect to internal network, which should be hidden.
//			if_nat = optarg;
//			break;
